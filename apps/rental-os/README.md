# Trailer Bros Rental OS — Version 1C Phase 1
...
See [Local operations](docs/LOCAL_OPERATIONS.md) for reset, seed, export/import, D1 recovery rehearsal, and the single fail-fast preflight command. See [Phase 3 readiness](docs/PHASE3_READINESS.md) for the private-by-design checklist and future prerequisites. Staging and production are intentionally unconfigured and must not be improvised.
...
SQLite and D1 run the checked-in overlap triggers inside atomic write batches, preventing two committed records from claiming the same trailer time. `wrangler.jsonc` has local development values and deliberately unusable staging/production placeholders; it contains no real account IDs, database IDs, hostnames, credentials, or secrets. `npm run d1:rehearse` applies all migrations to fresh local D1 and imports an existing SQLite data set while checking preservation of reservations, blackouts, audit events, and inspection-photo metadata.

Local authorization uses a development-only mock owner identity supplied by the loopback adapter. Staging/production mode verifies a Cloudflare Access JWT and restricts access to one configured owner email; mock authorization fails closed outside development. No Cloudflare resource has been created, and the placeholder environments are not deployable until separately approved Phase 3 configuration is supplied.
