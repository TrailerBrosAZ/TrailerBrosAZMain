...
const requiredTables=['trailers','customers','reservations','availability_blocks','payments','audit_events','condition_inspections','inspection_photos','cancellation_outcomes','deposit_decisions'];
...
const restoreOrder=['d1_migrations','trailers','customers','reservations','availability_blocks','payments','audit_events','condition_inspections','inspection_photos','cancellation_outcomes','deposit_decisions'];
...
export function verifyD1Backup(file:string):BackupVerification{
  ...
  try{
    ...
    const missingTables=requiredTables.filter(name=>!names.includes(name));const missingTriggers=requiredTriggers.filter(name=>!triggers.includes(name));
    ...
    const counts=Object.fromEntries(requiredTables.map(name=>[name,Number((db.prepare(`SELECT count(*) total FROM ${name}`).get() as {total:number}).total)]));
    return{sha256:createHash('sha256').update(sql).digest('hex'),bytes:Buffer.byteLength(sql),tableCount:requiredTables.length,triggerCount:requiredTriggers.length,counts,syntheticReservations:Number((db.prepare('SELECT count(*) total FROM reservations WHERE is_synthetic=1').get() as {total:number}).total),syntheticBlocks:Number((db.prepare('SELECT count(*) total FROM availability_blocks WHERE is_synthetic=1').get() as {total:number}).total)};
  }finally{db.close()}
}

if(import.meta.url===pathToFileURL(process.argv[1]||'').href){
  const file=process.argv[2];if(!file)throw new Error('Usage: npm run backup:verify -- <path-to-export.sql>');
  const result=verifyD1Backup(file);console.log(JSON.stringify({verified:true,...result}));
}
