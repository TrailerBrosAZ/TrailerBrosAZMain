...
const requiredTables=['trailers','customers','reservations','availability_blocks','payments','audit_events','condition_inspections','inspection_photos','cancellation_outcomes','deposit_decisions','booking_intents','agreement_templates','agreement_instances','pickup_condition_choices','booking_intent_conversions','secure_links','secure_link_attempts'];
...
const restoreOrder=['d1_migrations','trailers','customers','reservations','availability_blocks','booking_intents','delivery_quote_usage','secure_link_attempts','payments','agreement_templates','agreement_instances','pickup_condition_choices','booking_intent_conversions','secure_links','audit_events','condition_inspections','inspection_photos','cancellation_outcomes','deposit_decisions'];
...
export function verifyD1Backup(file:string,options:{preBookingFoundation?:boolean;preAgreementFoundation?:boolean;preConversionFoundation?:boolean;preSecureLinkFoundation?:boolean}={}):BackupVerification{
  ...
  try{
    ...
    const expectedTables=options.preBookingFoundation?requiredTables.filter(name=>!['booking_intents','booking_intent_conversions','secure_links','secure_link_attempts'].includes(name)):options.preAgreementFoundation?requiredTables.filter(name=>!['agreement_templates','agreement_instances','pickup_condition_choices','booking_intent_conversions','secure_links','secure_link_attempts'].includes(name)):options.preConversionFoundation?requiredTables.filter(name=>!['booking_intent_conversions','secure_links','secure_link_attempts'].includes(name)):options.preSecureLinkFoundation?requiredTables.filter(name=>!['secure_links','secure_link_attempts'].includes(name)):requiredTables;const missingTables=expectedTables.filter(name=>!names.includes(name));const missingTriggers=requiredTriggers.filter(name=>!triggers.includes(name));
