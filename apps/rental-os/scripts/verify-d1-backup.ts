...
const requiredTables=['trailers','customers','reservations','availability_blocks','payments','audit_events','condition_inspections','inspection_photos','cancellation_outcomes','deposit_decisions','booking_intents','agreement_templates','agreement_instances','pickup_condition_choices'];
...
const restoreOrder=['d1_migrations','trailers','customers','reservations','availability_blocks','booking_intents','delivery_quote_usage','payments','agreement_templates','agreement_instances','pickup_condition_choices','audit_events','condition_inspections','inspection_photos','cancellation_outcomes','deposit_decisions'];
...
export function verifyD1Backup(file:string,options:{preBookingFoundation?:boolean;preAgreementFoundation?:boolean}={}):BackupVerification{
  ...
  try{
    ...
    const expectedTables=options.preBookingFoundation?requiredTables.filter(name=>name!=='booking_intents'):options.preAgreementFoundation?requiredTables.filter(name=>!['agreement_templates','agreement_instances','pickup_condition_choices'].includes(name)):requiredTables;const missingTables=expectedTables.filter(name=>!names.includes(name));const missingTriggers=requiredTriggers.filter(name=>!triggers.includes(name));
...
if(import.meta.url===pathToFileURL(process.argv[1]||'').href){
  const file=process.argv[2];if(!file)throw new Error('Usage: npm run backup:verify -- <path-to-export.sql> [--pre-booking-foundation|--pre-agreement-foundation]');
  const result=verifyD1Backup(file,{preBookingFoundation:process.argv.includes('--pre-booking-foundation'),preAgreementFoundation:process.argv.includes('--pre-agreement-foundation')});console.log(JSON.stringify({verified:true,...result}));
}
