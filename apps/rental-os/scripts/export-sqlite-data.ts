    ...
    for (const row of rows)
      for (const value of Object.values(row))
        if (value instanceof Uint8Array)
          throw new Error(`Binary data export is not permitted (${table}).`);
    for (const row of rows) {
      const columns = Object.keys(row)
        .map((column) => `"${column}"`)
        .join(",");
      statements.push(
        `INSERT INTO "${table}" (${columns}) VALUES (${Object.values(row).map(quote).join(",")});`,
      );
    }
  }
...
if (process.argv[1]?.endsWith("export-sqlite-data.ts"))
  exportData(
    resolve(process.argv[2] || "data/rental-os.db"),
    resolve(process.argv[3] || "data/rental-os-export.sql"),
  );
