...
const preBookingFoundation=args.schema==='pre-booking-foundation';const preAgreementFoundation=args.schema==='pre-agreement-foundation';const preConversionFoundation=args.schema==='pre-conversion-foundation';const preSecureLinkFoundation=args.schema==='pre-secure-link-foundation';const preAgreementRenderer=args.schema==='pre-agreement-renderer';if(args.schema&&!preBookingFoundation&&!preAgreementFoundation&&!preConversionFoundation&&!preSecureLinkFoundation&&!preAgreementRenderer)throw new Error('Unsupported pre-migration schema value.');
...
const result=verifyD1Backup(output,{preBookingFoundation,preAgreementFoundation,preConversionFoundation,preSecureLinkFoundation,preAgreementRenderer});createRestorableD1Backup(output,restore);const restoreResult=verifyD1Backup(restore,{preBookingFoundation,preAgreementFoundation,preConversionFoundation,preSecureLinkFoundation,preAgreementRenderer});console.log(JSON.stringify({exported:true,file:output,restoreFile:restore,sha256:result.sha256,restoreSha256:restoreResult.sha256,bytes:result.bytes,counts:result.counts,syntheticReservations:result.syntheticReservations,syntheticBlocks:result.syntheticBlocks}));
