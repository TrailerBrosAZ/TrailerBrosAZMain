...
export function createLocalDatabasePort(db: Database.Database): DatabasePort {
  ...
  return {
    ...
    async first<T extends Record<string, unknown>>(
      ...
      return db.prepare(sql).get(...values(params)) as T | undefined;
    },
    async run(sql: string, params: SqlValue[] = []) {
      return run({ sql, params });
    },
    async batch(statements: SqlStatement[]) {
      return db.transaction(() => statements.map(run))();
    },
  };
}
...
export function migrate(db: Database.Database) {
  ...
  db.exec(
    "CREATE TABLE IF NOT EXISTS app_migrations (name text PRIMARY KEY NOT NULL, applied_at text NOT NULL DEFAULT (datetime('now')))",
  );
  if (
    hasReservations &&
    !db
      .prepare("SELECT 1 FROM app_migrations WHERE name=?")
      .get("0000_solid_demogoblin.sql")
  )
    db.prepare("INSERT INTO app_migrations(name) VALUES (?)").run(
  ...
  for (const name of [
    ...
    "0011_lumpy_black_cat.sql",
    "0012_breezy_thunderbolt_ross.sql",
    "0013_outgoing_randall_flagg.sql",
    ...
    if (db.prepare("SELECT 1 FROM app_migrations WHERE name=?").get(name))
      continue;
    const sql = readFileSync(resolve(`drizzle/${name}`), "utf8").replaceAll(
