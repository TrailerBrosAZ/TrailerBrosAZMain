...
export const bookingIntents = sqliteTable('booking_intents', {
  id: integer('id').primaryKey({ autoIncrement: true }), idempotencyKey: text('idempotency_key').notNull(), trailerId: integer('trailer_id').notNull().references(() => trailers.id), status: text('status', { enum: ['SUBMITTED','REVIEW_REQUIRED','EXPIRED'] }).notNull().default('SUBMITTED'),
