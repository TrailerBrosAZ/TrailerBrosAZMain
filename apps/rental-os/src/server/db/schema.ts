...
export const paymentLedgerEntries = sqliteTable('payment_ledger_entries', {
  id: integer('id').primaryKey({ autoIncrement: true }), reservationId: integer('reservation_id').references(() => reservations.id), bookingIntentId: integer('booking_intent_id'),
  kind: text('kind').notNull(), status: text('status').notNull(), amountCents: integer('amount_cents').notNull(), currency: text('currency').notNull().default('usd'), provider: text('provider').notNull().default('MOCK'), providerPaymentId: text('provider_payment_id'), providerRefundId: text('provider_refund_id'), idempotencyKey: text('idempotency_key').notNull(), reason: text('reason'), inspectionId: integer('inspection_id'), breakdownJson: text('breakdown_json').notNull().default('{}'), isSynthetic: integer('is_synthetic', { mode: 'boolean' }).notNull().default(true), createdAt: text('created_at').notNull().default(sql`(datetime('now'))`),
}, table => [uniqueIndex('payment_ledger_idempotency_idx').on(table.idempotencyKey)]);
export const paymentWebhookEvents = sqliteTable('payment_webhook_events', {
  id: integer('id').primaryKey({ autoIncrement: true }), providerEventId: text('provider_event_id').notNull(), eventType: text('event_type').notNull(), providerPaymentId: text('provider_payment_id'), processingStatus: text('processing_status').notNull(), sanitizedStatus: text('sanitized_status').notNull(), payloadHash: text('payload_hash').notNull(), errorCategory: text('error_category'), providerCreatedAt: text('provider_created_at').notNull(), processedAt: text('processed_at'), isSynthetic: integer('is_synthetic', { mode: 'boolean' }).notNull().default(true), createdAt: text('created_at').notNull().default(sql`(datetime('now'))`),
}, table => [uniqueIndex('payment_webhook_event_idx').on(table.providerEventId)]);
