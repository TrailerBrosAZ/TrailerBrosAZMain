...
const wizard=readFileSync(new URL('../src/client/CustomerBookingWizard.tsx',import.meta.url),'utf8');
...
describe('protected direct checkout UI contract',()=>{
 ...
 it('states that browser payment success is non-authoritative',()=>{expect(checkout).toContain('Signed server reconciliation—not the browser—controls confirmation');expect(checkout).toContain('Check payment status')});
 it('reconciles client errors before retry and finalizes only after authoritative collection',()=>{expect(checkout).toContain('recoverCheckoutAfterConfirmationError(onRefresh)');expect(checkout).toContain("recovery.retryAllowed");expect(checkout).toContain('The server confirms no payment was collected; you may retry.');expect(checkout).toContain("session?.state!=='PAYMENT_COLLECTED'");expect(checkout).toContain('/api/customer-preview/direct-checkout/finalize');expect(checkout).toContain('No additional action is needed.');expect(checkout).toContain("['PAYMENT_PENDING','PAYMENT_REQUIRED'].includes(session.state)")});
 ...
 it('uses a focused wizard with Rental OS calendar and customer-safe language',()=>{for(const text of ['Choose your rental time','The calendar is powered by the Rental OS','5,200 lb capacity','Your tow vehicle must be properly rated and equipped','Agreement and payment','Your reservation is confirmed'])expect(wizard).toContain(text);expect(wizard).not.toContain('published payload');expect(wizard).not.toContain('Request recorded');expect(wizard).not.toContain('Payment reconciled')});
 it('keeps payment last and uses server-authoritative automatic finalization',()=>{expect(wizard).toContain("['Availability','Your details','Review','Agreement & payment','Confirmed']");expect(wizard).toContain('Payment is the final customer action.');expect(checkout).toContain("session?.state!=='PAYMENT_COLLECTED'");expect(checkout).toContain('idempotencyKey:`checkout_convert_')});
});
