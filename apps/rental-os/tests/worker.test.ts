...
describe('Worker privacy boundary',()=>{
  ...
  it('protects operational diagnostics before the health route runs',async()=>{const response=await worker.fetch(new Request('https://private.example.test/api/health'),development());expect(response.status).toBe(401);expect(response.headers.get('cache-control')).toBe('no-store');});
  it('rejects unauthenticated non-POST webhook requests before assets or owner auth',async()=>{assets.fetch.mockClear();const response=await worker.fetch(new Request('https://private.example.test/api/payments/webhooks/stripe'),development());expect(response.status).toBe(405);expect(response.headers.get('cache-control')).toBe('private, no-store');expect(assets.fetch).not.toHaveBeenCalled()});
  it('fails closed for unsigned webhook POSTs while the signing secret is absent',async()=>{const response=await worker.fetch(new Request('https://private.example.test/api/payments/webhooks/stripe',{method:'POST'}),development());expect(response.status).toBe(503);expect(await response.json()).toEqual({error:'Webhook unavailable.'});expect(response.headers.get('cache-control')).toBe('private, no-store')});
