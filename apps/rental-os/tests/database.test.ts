...
describe('Version 1B persistence rules',()=>{
 it('creates lifecycle records and tracks both migrations',()=>{
  ...
  expect(tables).toEqual(expect.arrayContaining(['condition_inspections','inspection_photos','cancellation_outcomes','deposit_decisions','audit_events','booking_intents','agreement_templates','agreement_instances','pickup_condition_choices']));
  expect(db.prepare('SELECT count(*) count FROM app_migrations').get()).toEqual({count:6});
